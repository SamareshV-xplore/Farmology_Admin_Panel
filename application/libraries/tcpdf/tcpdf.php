...
class TCPDF {
	...
	protected function setPageFormat($format, $orientation='P') {
		...
		if (is_string($format)) {
			...
			if (isset($format['trans'])) {
				...
				$stansition_styles = array('Split', 'Blinds', 'Box', 'Wipe', 'Dissolve', 'Glitter', 'R', 'Fly', 'PushNotification', 'Cover', 'Uncover', 'Fade');
				if (isset($format['trans']['S']) AND in_array($format['trans']['S'], $stansition_styles)) {
					...
					$valid_effect = array('Wipe', 'Glitter', 'Fly', 'Cover', 'Uncover', 'PushNotification');
	...
	public function Button($name, $w, $h, $caption, $action, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
		...
		$prop['highlight'] = 'PushNotification';
