...
class Farm_management extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model("farm_management_model");
    ...
    public function farms_list()
    {
        if ($this->common_model->user_login_check()) {
            ...
            $header_data["title"] = "Farm Management | Farmology Admin Panel";
            $left_data["navigation"] = "farm-management";
            $left_data["sub_navigation"] = "farms-list";
            $page_data["list_of_farms"] = $this->farm_management_model->get_list_of_farms();
            
            $this->load->view("includes/header_view", $header_data);
            $this->load->view("includes/left_view", $left_data);
            $this->load->view("farm_management/farms_list", $page_data);
    ...
    public function report_subscriptions_list()
    {
        $header_data = array();
        $page_data = array();
        $left_data = array();
        ...
        $header_data['title'] = "Report Subscriptions List | Farm Management | Admin Panel";
        $left_data['navigation'] = "farm-management";
        $left_data["sub_navigation"] = "report-subscriptions-list";
        ...
        $page_data["reports_subscription_list"] = $this->farm_management_model->get_list_of_subscriptions();

        $this->load->view('includes/header_view', $header_data);
        $this->load->view('includes/left_view', $left_data);
        $this->load->view('farm_management/report_subscriptions_list', $page_data);
        $this->load->view('includes/footer_view', $footer_data);
    }

    public function crop_health_reports()
    {
        $header_data = array();
        $page_data = array();
        $left_data = array();
        ...
        $header_data['title'] = "Crop Health Reports | Farm Management | Admin Panel";
        $left_data['navigation'] = "farm-management";
        $left_data["sub_navigation"] = "crop-health-reports";
        ...
        $page_data['products'] = $this->farm_management_model->getAllProducts();

        // get request list
        $req_list = $this->farm_management_model->get_farm_report_requests($filter_data);
        $page_data['request_list'] = $req_list;

        $this->load->view('includes/header_view', $header_data);
        $this->load->view('includes/left_view', $left_data);
        $this->load->view('farm_management/crop_health_reports', $page_data);
        $this->load->view('includes/footer_view', $footer_data);
    }
    ...
    {
        if (!empty($report_id)) {
            $farm_details = $this->farm_management_model->get_farm_details_by_report_id($report_id);
    ...
    public function getProductByProductId()
    {
        ...
        $product = $this->farm_management_model->get_product_by_product_id($product_id);
    ...
    function get_recommended_products_list($product_id_list)
    {
        ...
        {
            $products_list[] = $this->farm_management_model->get_product_by_id($product_id);
        }
    ...
    function add_recommended_products($report_id, $product_id_list)
    {
        ...
        foreach($product_id_list as $product_id){
            $this->farm_management_model->add_recommended_product($report_id, $product_id);
        }
    }
    ...
    public function generate_report()
    {
        ...
        else
        {
            ...
            $pdf_upload_url = $this->farm_management_model->generate_report_pdf($report_id, $report_data, $expert_advice, $recommended_products);
    ...
    public function refresh_report_data($id)
    {
        ...
        if(count($report_data))
        {
            ...
            {
                $report_data = $this->farm_management_model->refresh_report_data($id);
                if($report_data!=null)
                {
                    $response = array(
                        "success" => true,
    ...
    {
        $crop_name = $this->farm_management_model->get_crop_name_by_farm_id($farm_id);
        if($crop_name!=null)
        {
            echo $crop_name;
        }
    ...
    {
        $sowing_date = $this->farm_management_model->get_sowing_date_by_farm_id($farm_id);
        if($sowing_date!=null)
        {
            echo $sowing_date;
        }
    ...
    {
        $is_deleted = $this->farm_management_model->delete_farm_and_reports($farm_id);
        if($is_deleted)
        {
    ...
    public function test_pdf_structure($template_name)
    {
        ...
        // $crop_name = $this->farm_management_model->get_crop_name_by_farm_id("58");
        // if($crop_name!=null)
        // {
        //     $data["crop_name"] = $crop_name;
        // }

        // $sowing_date = $this->farm_management_model->get_sowing_date_by_farm_id("58");
        // if($sowing_date!=null)
        // {
    ...
    // Ledger PDF Template Showing API
    // ==============================================================

    public function show_ledger_pdf_template()
    {
        include_once (APPPATH.'third_party/mpdf/vendor/autoload.php');
        $data = array();

        $HTML = $this->load->view("farm_management/ledger_pdf_template", $data, true);
        $PDF = new \Mpdf\Mpdf(['default_font' => 'Calibri']);
        $PDF->SetDisplayMode("fullpage");
        $PDF->AddPage();
        $PDF->WriteHTML($HTML);
