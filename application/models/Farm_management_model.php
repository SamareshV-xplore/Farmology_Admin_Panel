...
class Farm_management_model extends CI_Model {

    public function get_list_of_farms()
    {
        $SQL = "SELECT 
                NF.*, 
                CONCAT(U.first_name, ' ', U.last_name) AS username, 
                C.title AS crop 
                FROM FM_new_farms NF 
                LEFT JOIN FM_customer U ON U.id = NF.user_id 
                LEFT JOIN FM_crop C ON C.id = NF.crop_id 
                WHERE NF.status = 'A'";
        $result = $this->db->query($SQL)->result();
        return $result;
    }

    public function get_list_of_subscriptions ()
	{
		$condition = ["status" => "A"];
		$farm_data = $this->db->get_where("FM_new_farms", $condition)->result();
		foreach ($farm_data as $farm)
		{
			$condition = ["farm_id" => $farm->farm_id];
			$farm->paid_subscription = $this->db->select("*")
												->from("FM_report_subscription")
												->where($condition)
												->order_by("valid_to", "DESC")
												->get()->row();

			$condition2 = ["id" => $farm->user_id];
			$farm->farm_owner_data = $this->db->get_where("FM_customer", $condition2)->row();	
		}
