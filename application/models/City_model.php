...
class City_model extends CI_Model {
    ...
    function add_city($data)
    {
        ...
        $this->db->insert("FM_city_lookup", $insert_data);
        $id =  $this->db->insert_id();
        $response = array("status" => "Y", "message" => "New City added", "id" => $id);

        return $response;

    }

    function single_city_details($id = 0)
    {
        $details = array();
        $this->db->select("*");
        $this->db->from("FM_city_lookup");
        ...
        {
            $response = array("status" => "N", "message" => "No details found. Maybe city is already deleted.");
        }
        return $response;
    }
    ...
    function update_image($id, $image, $update_type)
    {
        ...
        $this->db->where("id", $id);
        $this->db->update("FM_city_lookup", $update_data);
        return true;
    ...
    function update_city($data)
    {
        ...
        {
            $response = array("status" => "N", "message" => "Invalid request. Maybe city already deleted.");
        }
        else
        {
            $update_data = array(
            ...
            $this->db->where("id", $id);
            $this->db->update("FM_city_lookup", $update_data);
            $response = array("status" => "Y", "message" => "City Details updated.");
    ...
    function delete_city_by_id($id)
    {
        ...
        if($query->num_rows() > 0)
        {
            ...
            $response = array("status" => "Y", "message" => "City successfully deleted.");

        }
        else
        {
            $response = array("status" => "N", "message" => "Invalid city ID or city already deleted.");
        }
        return $response;
    }
}
